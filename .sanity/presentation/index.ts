...
export const enableUrl = process.env.SANITY_STUDIO_VERCEL_ENV
  ? `https://${
      process.env.SANITY_STUDIO_VERCEL_ENV === 'production'
        ? 'certifikedu.sanity.build' // I don't understand why the primary domain doesn't have a variable
        : process.env.SANITY_STUDIO_VERCEL_BRANCH_URL?.replace('certifikedu-studio', 'certifikedu')
    }/api/draft`
